                    ...
                    for(Tier tier : tiers){
                        ...
                        for (int i = 0; i < jsonArray.length(); i++) {      
                            ...
                            for (int j = 0; j < jsonArray.length(); j++) {  
                                ...
                                //System.out.println(from_city + " <- from to ->" + to_city);
                                if (!from_city.equals(to_city)) {
                                    String item_name = to_city_object.getString("item_id").substring(3);
                                    String item_tier = to_city_object.getString("item_id").substring(0,2);                                    
                                    Double to_city_price = to_city_object.getDouble("buy_price_max");

                                    Value value_to_add = new Value(getItems_id_from_name(item_name,items), getCity_id_from_name(from_city, cities) , getCity_id_from_name(to_city, cities), getTier_id_from_name(item_tier,tiers), (to_city_price - from_city_price));
                                    ...
                                    try {                                    
                                        insert = conn.prepareStatement("INSERT INTO [Values] (items_id, from_city_id, to_city_id, tier_id, value, last_updated, rarity_id) VALUES (?, ?, ?, ?, ?, ?, 0);");
